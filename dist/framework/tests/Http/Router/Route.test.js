...
const Route_1 = require("../../../Http/Router/Route");
describe("Path normalization", () => {
    it("should return /", () => {
        expect(Route_1.Route.normalizePath("/")).toBe("/");
        expect(Route_1.Route.normalizePath("")).toBe("/");
        expect(Route_1.Route.normalizePath("//")).toBe("/");
    });
    it("should return /about/", () => {
        expect(Route_1.Route.normalizePath("/about")).toBe("/about/");
        expect(Route_1.Route.normalizePath("about")).toBe("/about/");
        expect(Route_1.Route.normalizePath("about/")).toBe("/about/");
        expect(Route_1.Route.normalizePath("/about/")).toBe("/about/");
        expect(Route_1.Route.normalizePath("/about//")).toBe("/about/");
...
describe("Route matching", () => {
    it("should match required parameters", () => {
        const route = new Route_1.Route("GET", "/products/view/{product}", () => { });
        expect(route.matches("GET", "/products/view/")).toEqual(false);
        expect(route.matches("GET", "/products/view/product-1")).toEqual(true);
        expect(route.matches("GET", "/products/view/product-1/")).toEqual(true);
        expect(route.matches("GET", "/products/view/product-1/extra")).toEqual(true);
    });
    it("should not match empty parameters", () => {
        const route = new Route_1.Route("GET", "/products/view/{}", () => { });
    ...
    it("should get required parameters names and values", () => {
        const route = new Route_1.Route("GET", "/products/view/{product}/id/{id}", () => { });
...
describe("Route can be named", () => {
    it("should be named", () => {
        const route = new Route_1.Route("GET", "/products/view/{product}", () => { }).setName("products.view");
