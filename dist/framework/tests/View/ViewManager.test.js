...
describe("Simple rendering", () => {
    ...
    it("should return input", () => {
        const viewManager = new ViewManager_1.ViewManager();
        viewManager.addPath(__dirname + "/TestTemplates");
        expect(viewManager.resolve("hello")).toEqual(`<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
  ...
  <body>
    ...
    it("should render a simple view", () => {
        const viewManager = new ViewManager_1.ViewManager();
        viewManager.addPath(__dirname + "/TestTemplates");
        expect(viewManager.resolve("hello-world", { name: "World" })).toEqual(`<h1>Hello World</h1>`);
    });
    it("should compile if statements", () => {
        const viewManager = new ViewManager_1.ViewManager();
        viewManager.addPath(__dirname + "/TestTemplates");
        expect(viewManager.resolve("single-if", { name: "World" })).toEqual("Hello World ");
    });
    it("should compile if statements with else", () => {
        const viewManager = new ViewManager_1.ViewManager();
        viewManager.addPath(__dirname + "/TestTemplates");
        expect(viewManager.resolve("if-else", { name: false })).toEqual("Hello Universe ");
    });
    it("should compile if statements with else if", () => {
        const viewManager = new ViewManager_1.ViewManager();
        viewManager.addPath(__dirname + "/TestTemplates");
        expect(viewManager.resolve("if-elseif", { name: false, age: 12 })).toEqual("Hello Universe ");
    });
